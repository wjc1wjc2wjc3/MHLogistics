...
+ (void) showLongTitleAlert:(NSString *) message inView:(UIView *) view {
    dispatch_async(dispatch_get_main_queue(), ^{
        prestrainView = view;
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:view ?:kDefaultView animated:YES];
        hud.label.text = message;
        ...
        CGRect rect = [message boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attr context:nil];
        CGRect labelFrame = hud.label.frame;
        CGFloat maxWidth = 200;
        CGFloat ratio =  rect.size.width / maxWidth;
        rect.size.width = rect.size.width > maxWidth ? maxWidth : rect.size.width;
        rect.size.height = rect.size.height * ratio;
        hud.label.numberOfLines = 0;
        hud.label.frame = CGRectMake(labelFrame.origin.x, labelFrame.origin.y, rect.size.width, rect.size.height);
        [hud hideAnimated:YES afterDelay:kShowTime];
    });
}
...
+ (void) showLoadingInView {
    UIView *view = nil;
    HZBitActivityIndicatorView *indicatorView = [HZBitActivityIndicatorView defaultIndicator];
    dispatch_async(dispatch_get_main_queue(), ^{
        prestrainView = view;
        ...
        [gloomyView addSubview:indicatorView];
...
+ (void) stopLoadingView {
    NSArray *subViews = [gloomyView subviews];
    for (UIView *view in subViews) {
        if ([view isKindOfClass:[HZBitActivityIndicatorView class]]) {
            HZBitActivityIndicatorView *indicatorView = (HZBitActivityIndicatorView *)view;
...
+ (void) showLongTitleAlert:(NSString *) alert {
    [self showLongTitleAlert:alert inView:nil];
}
