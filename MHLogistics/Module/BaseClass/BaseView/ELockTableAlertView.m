...
#import "ELockTableAlertView.h"
#import "AppDelegate.h"

#define kContentViewW (kScreenWidth * 0.8)
#define kContentViewMaxH (kScreenHeight * 0.7)  //内容最大高度
#define kTopViewHeight 50
#define rowHeight  50   //定义tableView行高
#define kBottomViewStyleNormalH 50   //底部只有，取消和确定按钮

@interface ELockSelectionCell : UITableViewCell
@property (nonatomic, strong) UIButton *selectionButton;
@property (nonatomic, strong) UILabel*  titleLabel;

@end

@implementation ELockSelectionCell

- (UIButton *)selectionButton{
    
    if (!_selectionButton) {
        _selectionButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_selectionButton setImage:[UIImage imageNamed:@"icon_lock_selected"] forState:UIControlStateSelected];
        [_selectionButton setImage:[UIImage imageNamed:@"icon_lock_selectedno"] forState:UIControlStateNormal];
        // 用代码添加约束时，该属性需要设置为no
...
@interface ELockTableAlertView() <UITableViewDelegate, UITableViewDataSource>
// Date
...
@implementation ELockTableAlertView

+ (ELockTableAlertView *_Nonnull)initWithTitle:(NSString *)title options:(NSArray *)optionsArray
                            singleSelection:(BOOL)selection
                              selectedItems:(NSArray *)items
                          completionHandler:(CompleteSelection)handler{
    
    UIWindow *window = ((AppDelegate*)([UIApplication sharedApplication].delegate)).window;
    ELockTableAlertView* view = [[ELockTableAlertView alloc]initWithFrame:window.bounds];
    ...
    [view.tableView registerClass:[ELockSelectionCell class] forCellReuseIdentifier:@"Cell"];
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        bgView.backgroundColor = [UIColor colorWithRed:0.72 green:0.72 blue:0.72 alpha:0.8];
...
- (UILabel *)titleLabel{
    ...
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = NAVIBAR_BLUE;
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitleColor: NAVIBAR_BLUE forState:UIControlStateNormal];
        ...
        // btn 间隔40
        NSLayoutConstraint *left = [NSLayoutConstraint constraintWithItem:_cancelBtn attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
        
        NSLayoutConstraint *bottom = [NSLayoutConstraint constraintWithItem:_cancelBtn attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeBottom multiplier:1 constant:0];
        
        NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:_cancelBtn attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1 constant:kContentViewW / 2.0];
        ...
        [centerView setBackgroundColor:VC_COLOR_BG];
        [_bottomView addSubview:centerView];
        
        [centerView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(weakSelf.bottomView.mas_top);
            make.centerX.mas_equalTo(weakSelf.bottomView.mas_centerX);
...
- (UIButton *)confirmBtn {
    if (!_confirmBtn) {
        ...
        [_confirmBtn setTitleColor: NAVIBAR_BLUE
        ...
        // btn间隔40，距离底部20
        NSLayoutConstraint *right = [NSLayoutConstraint constraintWithItem:_confirmBtn attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeRight multiplier:1 constant:0];
        
        NSLayoutConstraint *bottom = [NSLayoutConstraint constraintWithItem:_confirmBtn attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeBottom multiplier:1 constant:0];
        
        NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:_confirmBtn attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1 constant:kContentViewW / 2.0];
        
        NSLayoutConstraint *height = [NSLayoutConstraint constraintWithItem:_confirmBtn attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeHeight multiplier:1 constant:0];
...
- (void)closeView:(ELockTableAlertView *)view {
    [UIView animateWithDuration:0.3 animations:^{
        view.bgView.alpha = 0;
    } completion:^(BOOL finished) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ELockSelectionCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ELockSelectionCell *cell = (ELockSelectionCell *)[tableView cellForRowAtIndexPath:indexPath];
    if (self.isSingleSelection) {
        ...
        if (self.selectedArray && self.selectedArray.count > 0) {
            ...
            ELockSelectionCell *cell = (ELockSelectionCell *)[tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:[lastRow integerValue] inSection:0]];
            cell.selectionButton.selected = NO;
        }
        [self.selectedArray removeAllObjects];
        [self.selectedArray addObject:@(indexPath.row)];
